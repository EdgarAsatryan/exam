...
export default function OnSale({cartList, addToCart}) {
    ...
    <div className='w-full h-auto pb-16 bg-black flex items-center justify-center'>
      <div className='w-[80%] h-full flex flex-col gap-9'>
        ...
        <div className='flex lg:flex-col'>
          ...
          <div className='w-[36.3%] flex lg:self-start lg:w-[200%] relative'>
            ...
            {
              products?.filter((item) => item.onSale && filter.length > 0 ? item.category === filter:item.onSale)?.filter((item) =>  item.price >= priceFrom && item.price <= priceTo)?.map((val) => {
                  return (
